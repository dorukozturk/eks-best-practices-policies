...
def check_endpoint_public_access(region: str, cluster: str):
    ...
    endpoint_access = cluster_metadata["cluster"]["resourcesVpcConfig"]["endpointPublicAccess"]
    if endpoint_access:
        print(Fore.RED + 'Make the EKS Cluster Endpoint Private!')
        print(Style.DIM + 'https://aws.github.io/aws-eks-best-practices/security/docs/iam/#make-the-eks-cluster-endpoint-private')
...
def check_access_to_instance_profile(region: str, cluster: str):
    ...
    for instance in instance_metadata['Reservations']:
        if instance["Instances"][0]["MetadataOptions"]["HttpPutResponseHopLimit"] == 2:
            print(Fore.RED + f"Restrict access to the instance profile assigned to the worker node {instance['Instances'][0]['InstanceId']}")
            print(Style.DIM + 'https://aws.github.io/aws-eks-best-practices/security/docs/iam/#make-the-eks-cluster-endpoint-private')
